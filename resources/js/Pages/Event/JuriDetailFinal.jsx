...
const JURY_TYPE_LABELS = { pbb: 'PBB', vafor: 'Vafor' };
const JURY_TYPE_ORDER = ['pbb', 'vafor'];
...
export default function JuriDetailFinal({ event, contingent, finalScore, juryScores, juryMembers }) {
    const finalScores = juryScores?.final || {};

    return (
        <div className="min-h-screen bg-[#0D0C0A] text-[#F2EDD6] p-4 font-sans">
            <Head title={`Detail Juri Final - ${contingent.school_name}`} />
            <div className="max-w-5xl mx-auto space-y-6">

                <div className="flex items-center gap-3 pb-3 border-b border-bronze-muted/20">
                    <Link href={`/events/${event.slug}/leaderboard/final`}
                        className="text-xs text-text-muted hover:text-gold-light transition-all font-semibold uppercase tracking-wider shrink-0"
                    >
                        <ArrowLeft className="h-4 w-4 inline" /> The Final
                    </Link>
                ...
                {finalScore && (
                    <div className="premium-card border-bronze-muted/10 overflow-hidden">
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 p-4">
                            {[
                                { label: 'PBB', value: finalScore.pbb_score, color: 'text-gold-bright' },
                                { label: 'Danton', value: finalScore.danton_score, color: 'text-gold-light' },
                                { label: 'Vafor', value: finalScore.vafor_score, color: 'text-sky-400' },
                                { label: 'Voting Bonus', value: finalScore.voting_bonus, color: 'text-violet-400' },
                                { label: 'Penalti', value: `-${finalScore.penalties}`, color: 'text-accent-mahogany' },
                                { label: 'Total', value: finalScore.total_score, color: 'text-emerald-400' },
                        ...
                        <div className="px-4 pb-4 text-center">
                            <span className="text-[10px] text-text-muted uppercase tracking-wider font-bold">Skor Final </span>
                            <span className="text-xl font-extrabold text-gold-primary font-mono">{Math.round(finalScore.total_score)}</span>
                        </div>
                    </div>
                )}
                ...
                {Object.keys(finalScores).length === 0 && (
                    <div className="premium-card p-12 border-accent-maroon/30 text-center">
                        <p className="text-text-muted text-sm">Belum ada data penilaian juri final untuk kontingen ini.</p>
                    </div>
                )}

                {JURY_TYPE_ORDER.map(juryType => {
                    const typeScores = finalScores[juryType];
