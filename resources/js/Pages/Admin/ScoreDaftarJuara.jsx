...
import { Trophy, Award, Info, Download } from 'lucide-react';
import { calculateChampions } from '../../Utils/champions';
import { categoryLabels } from '../../Utils/scoreUtils';
import StatusBadge from '../../Components/StatusBadge';
import ScrollReveal from '../../Components/ScrollReveal';

export default function ScoreDaftarJuara({ event, contingents = [], scores = [], finalRoundScores = [] }) {
    const [activeCategory, setActiveCategory] = useState('U16');

    const hasActiveScores = scores.some(
        s => s.contingent?.category_type === activeCategory && parseInt(s.grand_total) > 0
    );
    const hasSocialLikes = contingents.some(
        c => c.category_type === activeCategory && (parseInt(c.reels_likes) > 0 || parseInt(c.posts_likes) > 0)
    );
    const showContent = hasActiveScores || hasSocialLikes;

    const { brackets, awards } = calculateChampions(activeCategory, contingents, scores, finalRoundScores);
    ...
    const formulaInfo = {
        U12: {
            title: 'Juara Umum SD',
            formula: 'Nilai Seleksi = PBB + Danton + Vafor + Kostum + Makeup − Penalti',
            desc: 'Juara Umum ditentukan oleh Nilai Seleksi tertinggi di babak penyisihan.',
        },
        U16: {
            title: 'Juara Grand Final SMP',
            formula: 'Skor Final = PBB + Danton + Vafor − Penalti (penampilan baru)',
            desc: '2 finalis dengan Nilai Seleksi tertinggi (PBB+Danton+Vafor+Kostum+Makeup−Penalti) bertanding di babak Grand Final.',
        },
        U19: {
        ...
        Purna: {
            ...
            desc: 'Kategori Purna tidak mengikuti babak Grand Final.',
        },
    }[activeCategory];

    return (
        <div className="min-h-screen bg-[#0D0C0A] text-[#F2EDD6] p-6 font-sans">
            <Head title="Daftar Juara" />
            ...
            <div className="max-w-6xl mx-auto space-y-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-bronze-muted/20 pb-6">
                    <div>
                        <span className="text-[10px] font-bold tracking-widest text-gold-light bg-accent-maroon/20 px-2 py-0.5 border border-gold-primary/20 rounded-full uppercase">
                            Champions List / Daftar Juara
                        </span>
                        <h1 className="text-3xl font-extrabold text-white mt-2">
                            Daftar <span className="text-gold-primary">Juara</span>
                        </h1>
                        <p className="text-xs text-text-muted mt-1">{event.name}</p>
                    </div>
                    <div className="flex items-center gap-2 mt-4 md:mt-0">
                        <a href={`/admin/events/${event.slug}/scores/daftar-juara/export`}
                    ...
                    {['U12', 'U16', 'U19', 'Purna'].map((cat) => (
                        <button key={cat} type="button" onClick={() => setActiveCategory(cat)}
                            className={`flex-1 py-1.5 text-center rounded text-xs font-bold transition-all uppercase ${
                                activeCategory === cat ? 'bg-gradient-to-r from-gold-primary to-gold-bright text-deep-black shadow font-extrabold' : 'text-bronze-muted hover:text-white'
                            }`}>
                ...
                {!showContent ? (
                    <div className="premium-card border border-bronze-muted/20 p-8 text-center animate-fade-in">
                        <Trophy className="h-12 w-12 text-bronze-muted/40 mx-auto mb-3" />
                        <h3 className="text-lg font-bold text-text-muted">Belum Tersedia Juara</h3>
                        <p className="text-sm text-bronze-muted mt-1">
                            Data belum tersedia untuk kategori {categoryLabels[activeCategory] || activeCategory}. Input nilai juri di halaman Rekap atau input likes Instagram di menu Social Media.
                        </p>
                    </div>
                ) : (<>
                {hasActiveScores ? (<>
                        ...
                        <div className="bg-gradient-to-r from-gold-primary/10 via-gold-bright/5 to-transparent p-6 border-b border-gold-primary/20">
                            ...
                            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                                <div className="flex items-center gap-4">
                                    {championBracket.team.logo_path && (
                                        <img src={championBracket.team.logo_path} alt="" className="h-14 w-14 rounded-full object-cover border-2 border-gold-primary/30" />
                                    )}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <div className="bg-deep-black/40 rounded p-3 border border-bronze-muted/10">
                                    ...
                                    <div className="text-lg font-bold text-white font-mono">{Math.round(isFinalRound ? championBracket.team.finalPbb : championBracket.team.pbb)}</div>
                                </div>
                                <div className="bg-deep-black/40 rounded p-3 border border-bronze-muted/10">
                                    <div className="text-[9px] text-text-muted uppercase tracking-wider">Danton</div>
                                    <div className="text-lg font-bold text-white font-mono">{Math.round(isFinalRound ? championBracket.team.finalDanton : championBracket.team.danton)}</div>
                                </div>
                                <div className="bg-deep-black/40 rounded p-3 border border-bronze-muted/10">
                                    <div className="text-[9px] text-text-muted uppercase tracking-wider">Vafor</div>
                                    <div className="text-lg font-bold text-white font-mono">{Math.round(isFinalRound ? championBracket.team.finalVafor : championBracket.team.vafor)}</div>
                                </div>
                                <div className="bg-deep-black/40 rounded p-3 border border-bronze-muted/10">
                                    <div className="text-[9px] text-text-muted uppercase tracking-wider">Kostum (Net)</div>
                                    <div className="text-lg font-bold text-white font-mono">{Math.round(championBracket.team.kostumNet)}</div>
                                </div>
                                <div className="bg-deep-black/40 rounded p-3 border border-bronze-muted/10">
                                    <div className="text-[9px] text-text-muted uppercase tracking-wider">Makeup</div>
                                    <div className="text-lg font-bold text-white font-mono">{Math.round(championBracket.team.makeup)}</div>
                                </div>
                                <div className="bg-deep-black/40 rounded p-3 border border-accent-mahogany/20">
                                    ...
                                    <div className="text-lg font-bold text-accent-mahogany font-mono">{championBracket.team.penalties > 0 ? `-${Math.round(championBracket.team.penalties)}` : '0'}</div>
                                </div>
                            </div>
                                ...
                                <div key={idx} className={`premium-card border overflow-hidden ${isChampion ? 'border-gold-primary/30' : 'border-bronze-muted/20'}`}>
                                    <div className={`p-4 border-b ${isChampion ? 'bg-gradient-to-r from-gold-primary/10 to-transparent border-gold-primary/20' : 'bg-white/5 border-bronze-muted/10'}`}>
                                        ...
                                        <div className="flex items-center gap-3">
                                            {team.logo_path && (
                                                <img src={team.logo_path} alt="" className="h-10 w-10 rounded-full object-cover border border-gold-primary/20" />
                                            )}
                                            <div>
                                    ...
                                    <div className="p-4 space-y-3">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            ...
                                            {Math.round(team.finalObj?.voting_bonus || 0) > 0 && (
                ...
                {!championBracket && !isFinalRound && (
                    <div className="premium-card border border-bronze-muted/20 p-6 text-center">
                        <p className="text-sm text-text-muted">Tidak ada Juara Umum untuk kategori {activeCategory === 'Purna' ? 'Purna' : activeCategory}.</p>
                    </div>
                )}
                ...
                <div className="premium-card overflow-hidden border border-gold-primary/20">
                    <div className="p-4 bg-gold-primary/5 border-b border-gold-primary/20 flex justify-between items-center">
                        <h3 className="font-bold text-white text-sm flex items-center gap-2">
                            <Trophy className="h-4.5 w-4.5 text-gold-primary" /> Papan Juara Peringkat ({categoryLabels[activeCategory] || activeCategory})
                        </h3>
                                                    ...
                                                    <td className="p-3">
                                                        {hasTeam ? (() => {
                                                            ...
                                                            return (
                                                                <div className="flex items-center gap-3">
                                                                    {item.team.logo_path && (
                                                                        <img src={item.team.logo_path} alt="" className="h-9 w-9 rounded-full object-cover border border-white/10 shrink-0" />
                                                                    )}
                    ...
                    <div className="premium-card border border-bronze-muted/20 p-6 text-center animate-fade-in">
                        <p className="text-sm text-text-muted">
                            Belum ada nilai juri untuk kategori {categoryLabels[activeCategory] || activeCategory}. Hanya penghargaan berbasis sosial media yang tersedia.
                        </p>
                    </div>
                ) : null}
                            ...
                            <tbody className="divide-y divide-bronze-muted/10 text-white/90">
                                {awards.length === 0 ? (
                                    ...
                                    if (!award.metricLabel) {
                                        ...
                                        if (award.name.includes('Kostum')) metricLabel = 'Kostum Net';
                                                        ...
                                                        {hasTeam ? (
                                                            <div className="flex items-center gap-2">
                                                                {award.team.logo_path && (
                                                                    <img src={award.team.logo_path} alt="" className="h-7 w-7 rounded-full object-cover border border-white/10 shrink-0" />
                                                                )}
                                                                <span className="font-bold text-gold-light">{award.team.school_name}{award.team.is_reguler ? ' *reguler' : ''}</span>
                                                            </div>
                ...
                <div className="premium-card overflow-hidden border border-bronze-muted/20">
                    <div className="p-4 bg-white/5 border-b border-bronze-muted/10">
                        <h3 className="font-bold text-white text-sm flex items-center gap-2">
                            <Info className="h-4.5 w-4.5 text-gold-primary" /> Aturan Tie-Breaking (Saat Nilai Sama)
                        </h3>
                    </div>
                    <div className="overflow-x-auto text-xs scroll-smooth">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-bronze-muted/20 bg-deep-black/60 text-[9px] uppercase tracking-wider text-text-muted">
                                    <th className="p-3">Kategori Penilaian</th>
                                    <th className="p-3">Variabel Penentu Tertinggi</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-bronze-muted/10 text-white/90">
                                {[
                                    ['Umum Garda', 'PBB'],
                                    ['Peringkat', 'PBB'],
                                    ['PBB Terbaik', 'Berkumpul + Periksa Kerapian'],
                                    ['Danton Terbaik', 'Penguasaan Materi + Lapangan'],
                                    ['Variasi Formasi Terbaik', 'Tingkat Kesulitan & Detail Gerakan + Penjiwaan, Ekspresi, Pengucapan Kalimat'],
                                    ['Kostum Terbaik', 'Kreativitas Bentuk & Perpaduan Warna Dengan Kostum'],
                                    ['Make Up Terbaik', 'Kesesuaian Make Up dengan Desain Kostum'],
                                    ['Kontingen Terbaik, Sponsor Terbaik, Kreator Terfavorit, Peserta Terfavorit, Supporter Terfavorit', 'Riwayat tercepat mencapai nilai tertinggi'],
                                ].map((row, idx) => (
                                    <tr key={idx} className="hover:bg-white/[0.01]">
                                        <td className="p-3 font-semibold text-white">{row[0]}</td>
                                        <td className="p-3 text-text-muted">{row[1]}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
                    <div className="p-3 bg-deep-black/40 border-t border-bronze-muted/10">
                        <p className="text-[10px] text-text-muted leading-relaxed">
                            <span className="text-gold-light font-bold">Catatan:</span> Apabila terdapat kesamaan pada hasil akhir juara,
                            maka akan diambil dari nilai variabel penentu tertinggi sesuai tabel di atas.
                            Keputusan nilai juri bersifat mutlak, tidak dapat diganggu gugat dan dapat dipertanggungjawabkan.
                        </p>
                    </div>
                </div>
                </ScrollReveal>
